...
class ScanKPU
{
	...
	public function getDataKPUAndKawalPemilu($id)
	{
		...
		if($countParam > 4) return;
		
		$fetchDataKPU = $this->fetchDataKPU($id);

		if($fetchDataKPU == false) return;

		$fetchNamaWilayah = $this->nameWilayah($id);
		if($countParam == 4) $fetchKawalPemilu = $this->fetchKawalPemilu(end($extract));
	...
	private function compareData($kpu, $kawalPemilu, $id, $key)
	{
		...
		if($extractKpu['jokowi'] == null && $extractKpu['prabowo'] == null) return;

		if(!isset($kawalPemilu->pas2) && !isset($kawalPemilu->pas1)) return;

		if($kawalPemilu->pas2 == false && $kawalPemilu->pas1 == false) return;
	...
	{
		$extract = explode('/', $id);
		if(!file_exists("{$this->dataWilayah[0]}")) mkdir("{$this->dataWilayah[0]}");
...
------\n\n";
	...
	public function fetchKawalPemilu($id)
	{
		...
		if(isset(json_decode($getContent)->data)) return json_decode($getContent)->data;
	}
	...
	public function fetchDataKPU($id = false)
	{
		...
		if(isset(json_decode($getContent)->table)) return json_decode($getContent)->table;
	}
